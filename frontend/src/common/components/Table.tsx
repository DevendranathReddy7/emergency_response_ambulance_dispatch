...
const DynamicTable = ({ data, updateDetails, deleteDetails }: TableProps) => {
   ...
   // const [modalData, setModalData] = useState<any>(null);
    ...
    const showActionButtons = (value: string[], row: any) => {
        return (
            <div className="flex justify-center space-x-2">
                {value.map((action, i) => (
                    <button
                        key={i}
                        className={`px-3 py-1 rounded-sm text-white ${action === 'View' ? 'bg-blue-500 hover:bg-blue-600' : action === 'Delete' ? 'bg-red-700 hover:bg-red-400' : 'bg-blue-800 hover:bg-blue-400'}`}
    ...
    const showBgForPriority = (value: any) => {
    ...
    const renderCell = (value: any, col: string, row: any) => {
        if (col === 'Actions' && Array.isArray(value)) {
            return showActionButtons(value, row)
        }
        if (col === 'Priority') {
            return showBgForPriority(value)
            ...
            <div className="hidden md:block overflow-x-auto rounded-md">
                <table className="border-collapse min-w-full">
                    <thead>
                        <tr>
                            {columns.map((col, i) => (
                                <th key={i} className="text-center p-2 bg-blue-800 border-b text-white border-gray-300">
                                    {col}
                                </th>
                            ))}
                        </tr>
                                ...
                                {(row as any).Actions.map((action: string, i: number) => (
                                    <button
                                        ...
                                        className={`px-4 py-2 rounded-md text-white text-sm font-medium
                                            ${action === 'View' ? 'bg-blue-500 hover:bg-blue-600' : action === 'Delete' ? 'bg-red-700 hover:bg-red-400' : 'bg-blue-800 hover:bg-blue-400'}`}
                                        onClick={() => handleAction(action, row.id, row)}
                                    >
                                        {action}
                                    </button>
